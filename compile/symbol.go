...
func defineOuterFuncConst(id string, outerFunc reflect.Value, scope *GlobalScope) Symbol {
    constSymbol := &ConstSymbol{
        Name:  fmt.Sprintf("%s::%s", consts2.ConstOuterFunc, id),
        Kind:  consts2.ConstOuterFunc,
        Value: outerFunc,
    }
    cSymbol, _ := scope.DefineOrGetConst(constSymbol)
    return cSymbol
