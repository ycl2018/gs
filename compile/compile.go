...
func (s *StackCompileVisitor) VisitFloatAtom(ctx *gen.FloatAtomContext) interface{} {
    ...
    s.Write(consts.InstrFConst, ctx.GetStart(), symbol.GetAddress())
    return nil
}
...
func (s *StackCompileVisitor) VisitStringAtom(ctx *gen.StringAtomContext) interface{} {
    ...
    s.Write(consts.InstrSConst, ctx.GetStart(), symbol.GetAddress())
    return nil
}
...
func (s *StackCompileVisitor) VisitCopyCall(ctx *gen.CopyCallContext) interface{} {
    ctx.Expr().Accept(s)
    s.Write(consts.InstrCopy, ctx.GetStart())
    return nil
}
...
func (s *StackCompileVisitor) VisitLogicalOrExpr(ctx *gen.LogicalOrExprContext) interface{} {
    ...
    for i, andExpr := range allLogicalAndExpr {
        andExpr.Accept(s)
        if i != 0 {
            s.Write(consts.InstrOR, ctx.GetStart())
        }
...
func (s *StackCompileVisitor) VisitLogicalAndExpr(ctx *gen.LogicalAndExprContext) interface{} {
    ...
    for i, cmpExpr := range allComparisonExpr {
        cmpExpr.Accept(s)
        if i != 0 {
            s.Write(consts.InstrAND, ctx.GetStart())
        }
    }
    return nil
}
...
func (s *StackCompileVisitor) VisitConstKeyEntry(ctx *gen.ConstKeyEntryContext) interface{} {
    ...
    case gen.GsLexerSTRING:
        s.Write(consts.InstrSConst, ctx.GetStart(), s.defineStringConst(ctx.STRING().GetText()[1:len(ctx.STRING().GetText())-1]))
    case gen.GsLexerINT:
    ...
    case gen.GsLexerFLOAT:
        ...
        s.Write(consts.InstrFConst, ctx.GetStart(), defineFloatConst(val, s.GlobalScope).GetAddress())
