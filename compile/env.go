    ...
    for i := 0; i < len(accessors); i++ {
        ...
        case *gen.PropertyAccessContext:
            ...
            case reflect.Struct:
                ...
                if m != nil {
                    s.Write(consts.InstrConst, a.GetStart(), defineOuterFuncConst(indexId.String(), m.Func, s.GlobalScope).GetAddress())
                    return
                }
                // field
                f, err := s.Env.IndexField(fieldName, curType)
            ...
            case reflect.Interface:
                // interface Load
                if i == len(accessors)-1 {
                    s.Write(consts.InstrMLoad, a.GetStart(), defineStringConst(fieldName, s.GlobalScope).GetAddress())
                } else {
                    s.Write(consts.InstrFLoad, a.GetStart(), defineStringConst(fieldName, s.GlobalScope).GetAddress())
                }
            default:
